...
| `Astro & Marine` | night-photography and surf go/no-go planning — `/astro/window` (deterministic per-night verdict), `/astro/sites` (candidate drive-to sites with a Bortle baseline), `/marine/window` (per-day surf session verdict over the same engine) and `/marine/spots`                                      |
