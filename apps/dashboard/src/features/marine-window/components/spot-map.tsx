...
// The four spots span Italy to Portugal, so the initial view (before the first fitBounds) sits
// mid-Atlantic-coast rather than over any single spot.
const DEFAULT_CENTER: [number, number] = [-2, 42]
...
export default function SpotMap({
  ...
  // Markers: one per spot, the selected one visually distinct. Re-synced whenever the spot list
  ...
  useEffect(() => {
    ...
    const addMarkers = () => {
      ...
      markersRef.current = data.data.map((spot) => {
        const el = document.createElement('button')
        el.type = 'button'
        el.setAttribute('aria-label', `Select ${spot.name}`)
        el.style.width = '12px'
        ...
        el.style.background = spot.id === spotId ? VX.accentFill : VX.muted
        el.addEventListener('click', () => onSelectSpotRef.current(spot.id))
        return new Marker({ element: el }).setLngLat([spot.lon, spot.lat]).addTo(map)
      })
    }
  ...
  // Fit bounds to every spot once, on first load only.
  useEffect(() => {
    const map = mapRef.current
