> **Superseded 2026-07-11.** This was Argo's hand-rolled Blueprint-palette DESIGN.md (the
> greenfield POC referenced in its own "Promotion path" section below). It has been replaced by
> `basalt-ui` as the design system — see the current `/DESIGN.md` (now a thin app-delta record) and
> `.claude/rules/basalt-*.md`. Kept here as historical reference only; do not follow its guidance.
...
# (reskinned from BP) + the (retired) local charts package's primitives; recurring few, inventoried.
...
Argo's signature surface — the visx primitives in the (retired) local charts package. Full contract
lived in that package's own `CLAUDE.md` + `~/.claude/rules/visx-charts.md`. The design rules:
