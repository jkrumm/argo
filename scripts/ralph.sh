#!/usr/bin/env bash
# Hermes Chat — RALPH Loop Runner (Phase A: core chat, Groups 1–6)
#
# Usage:
...
#
# Logs: .ralph-logs/group-N.log   (watch live: tail -f .ralph-logs/group-N.log)
#
# Prerequisites:
#   brew install coreutils                 # for gtimeout
#   claude CLI in PATH
#   Hermes API server enabled (Group 0, manual) + dev infra up:
#     cd ~/SourceRoot/vps && make up       # Postgres + ClickStack + Valkey
#   1Password app unlocked (op --account tkrumm)
...
# Model + transport. Opus 4.8 / high / Max for this loop (novel integration work;
# explicitly NOT routed to the DeepSeek bridge). Override via env at launch.
...
GROUP_TITLES=(
  ...
  "Hermes chat proxy + persistence"
  "General AI gateway (/ai/v1/*)"
  "Read CRUD + DeepSeek titling"
  "Frontend core chat (working chat)"
  "Smart cards + rich rendering"
)
...
refuse_default_branch() {
  ...
  case "$current" in
    master|main)
      ...
      log_error "Switch to a feature branch first: git checkout -b feat/hermes-chat"
      exit 1 ;;
  esac
...
require_op_session() {
  log_info "Verifying 1Password CLI session (op --account $OP_ACCOUNT)..."
  if ! gtimeout 5 op whoami --account "$OP_ACCOUNT" >/dev/null 2>&1; then
    log_error "1Password CLI session is not active."
    log_error "Sign in once before launching: eval \$(op signin --account $OP_ACCOUNT)"
    exit 1
  fi
...
prefetch_secrets() {
  ...
  db_password="$(gtimeout 30 op read 'op://vps/argo/DB_PASSWORD' --account "$OP_ACCOUNT" 2>/dev/null || true)"
  postgres_db="$(gtimeout 30 op read 'op://vps/config/POSTGRES_DB' --account "$OP_ACCOUNT" 2>/dev/null || true)"
  api_secret="$(gtimeout 30 op read 'op://common/api/SECRET' --account "$OP_ACCOUNT" 2>/dev/null || true)"
...
EOF
  ...
  log_success "Secrets cached to .ralph-secrets.env (mode 600) and exported (DATABASE_URL ready)."
}
remove_secrets() {
...
install_push_guard() {
  ...
  if [[ -f "$hook" ]]; then mv "$hook" "${hook}.ralph-backup"; fi
  cat > "$hook" <<'HOOK'
#!/usr/bin/env bash
...
acquire_lock() {
  ...
  echo "$$" > "$LOCK_FILE"; LOCK_ACQUIRED=true
...
# ── State management (quoted heredocs + argv only) ────────────────────────────
...
state_file = sys.argv[1]; titles = sys.argv[2:]
...
json.dump({"groups": groups, "created_at": now}, open(state_file, "w"), indent=2)
...
for g in state['groups']:
    if g['id'] == gid: print(g.get(field, '')); break
PYEOF
}
...
state = json.load(open(state_file))
for g in state['groups']:
    if g['id'] == gid: g['attempts'] = g.get('attempts', 0) + 1; break
json.dump(state, open(state_file, 'w'), indent=2)
PYEOF
}
...
state = json.load(open(state_file))
for g in state['groups']:
    if g['id'] == gid: g['attempts'] = max(0, g.get('attempts', 0) - 1); break
json.dump(state, open(state_file, 'w'), indent=2)
PYEOF
}
...
print(f"  {total} groups | {done} complete | {total-done-blocked} pending | {blocked} blocked\n")
for g in state['groups']:
    a = f"  (attempt {g['attempts']})" if g['attempts'] > 0 else ""
    print(f"  {icons.get(g['status'],'⬜')}  Group {g['id']}: {g['title']}{a}")
PYEOF
}

# ── Validation (no DB/op; tests run inside groups via DATABASE_URL) ───────────
...
run_group() {
  ...
  log_info "Claude running (model: $RALPH_MODEL, effort: $RALPH_EFFORT, transport: $RALPH_TRANSPORT, timeout: ${CLAUDE_TIMEOUT}s) → .ralph-logs/group-$group_id.log"
  ...
  if env -u ANTHROPIC_API_KEY "${group_env[@]}" gtimeout "$CLAUDE_TIMEOUT" claude \
    ...
    < /dev/null > "$log_file" 2>&1; then exit_code=0; else exit_code=$?; fi
...
import json, sys, datetime
state = json.load(open(sys.argv[1])); report_file = sys.argv[2]
icons = {'complete': '✅', 'blocked': '🚫', 'pending': '⬜', 'in_progress': '🔄'}
total = len(state['groups'])
done = sum(1 for g in state['groups'] if g['status'] == 'complete')
blocked = sum(1 for g in state['groups'] if g['status'] == 'blocked')
now = datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
lines = ["# RALPH Report — Hermes Chat (Phase A)", "", f"Generated: {now}",
         f"Groups: {total} total | {done} complete | {total-done-blocked} pending | {blocked} blocked",
         "", "## Status", ""]
for g in state['groups']:
    a = f" (attempts: {g['attempts']})" if g['attempts'] > 0 else ""
    lines.append(f"- {icons.get(g['status'],'⬜')} **Group {g['id']}**: {g['title']}{a}")
lines += ["", "## Next Steps", ""]
if done == total:
    lines += ["Phase A complete.", "", "1. `git log --oneline -20`",
              "2. Manual QA the running chat (Mac + iPhone)",
              "3. Scaffold Phase B (Groups 7–10) and run."]
else:
...
main() {
  ...
  echo ""; echo -e "${BOLD}  RALPH Loop — Hermes Chat (Phase A)${NC}"; echo ""
  require_commands
  cd "$REPO_ROOT"
  init_state
  ...
  for group_id in "${groups_to_run[@]}"; do
    ...
    [[ "$status" == "complete" ]] && { echo -e "  ✅  Group $group_id: ${GROUP_TITLES[$group_id]} — skipped (complete)"; continue; }
    [[ "$status" == "blocked" ]] && { echo -e "  🚫  Group $group_id: ${GROUP_TITLES[$group_id]} — skipped (blocked)"; continue; }

    local attempts; attempts=$(get_field "$group_id" "attempts")
    if [[ "$attempts" -ge "$MAX_RETRIES" ]]; then
      log_warn "Group $group_id reached max retries. Marking blocked."
      set_field "$group_id" "status" "blocked"; continue
    fi
    ...
    if [[ "$group_id" -gt 1 ]]; then
      if ! validate "pre-group $group_id"; then
        log_error "Pre-group validation failed. Fix before continuing."; exit 1
      fi; echo ""
    fi
    ...
    local run_result=0
    run_group "$group_id" || run_result=$?; echo ""
    ...
    if [[ $run_result -eq 0 ]]; then
      ...
      else log_warn "Post-group validation FAILED. Review log and fix."; log_warn "Retry: ./scripts/ralph.sh --reset $group_id"; fi
      ...
      set_field "$group_id" "status" "pending"; dec_attempts "$group_id"
      log_error "Group $group_id paused: Claude usage/session limit reached."
      log_error "Re-run ./scripts/ralph.sh once the limit resets — it resumes here."; break
    else
      ...
      local new_attempts; new_attempts=$(get_field "$group_id" "attempts")
