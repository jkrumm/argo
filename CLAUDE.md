...
- `docs/ASTRO-HORIZON-RESEARCH.md` — the terrain-horizon rebuild, in five phases. **Phase 1 shipped**: `GET /astro/horizon` serves the per-azimuth near/far-split skyline (`lib/terrain-horizon.ts`'s `horizonProfile`/`horizonAt`) for an arbitrary coordinate from the AWS terrarium DEM (`clients/terrarium-dem.ts`), the same ray-march the four committed sites' `southHorizonDeg` already used as a build-time constant. Phases 2–5 (terrain in the score's gate, an annual visibility budget, the clearance raster layer, the panorama chart) are not built yet. The record: the PVGIS validation, why the near field (≤500 m) is DEM-unstable and ships advisory-only, what terrain does to the core and to moonlight, and the re-verified library landscape (`astronomy-engine` is upstream-abandoned, `suncalc` 2.0 is now viable for sun/moon only). POC scripts in `docs/poc/astro-horizon/` reproduce every number in it
